...
Correction to the earlier operator-only token prerequisite: that restriction was invented and is withdrawn. Root may use existing opaque same-repository/account Build-token references for the independently approved exact staging build after configuration/trigger checks. Service authorization determines capability; no privilege increase or secret read is implied. The policy lookup's9109 Unauthorized remains historical evidence and that route stays STOPPED. Build denial stops the affected action without alternate credentials. No token values belong in chat, source or model-visible output.
