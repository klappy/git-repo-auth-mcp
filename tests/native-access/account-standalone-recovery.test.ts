...
it('trusted terminal identity survives expired session; mismatch and revoked or malformed binding deny', async () => {
  for (const fault of ['wrong-id', 'revoked', 'missing-epoch', 'none']) {
    ...
    if (fault === 'revoked' || fault === 'missing-epoch') await expect(store.begin(browser, active.handle, c.ref, true)).rejects.toThrow();
    else if (fault === 'wrong-id') await expect(issue(store, c.ref, true, active.handle, 2002)).rejects.toThrow();
    else { const recovered = await issue(store, c.ref, true, active.handle); expect((await store.load(recovered.handle)).identity.githubId).toBe(1001); await expect(store.loadContinuation(browser, c.ref)).rejects.toThrow(); }
  }
});
...
it('native rendered standalone login covers missing, retired, expired and spent refs, and Cancel during provider exchange', async () => {
  ...
  let entered!: () => void, release!: () => void, pause: Promise<void> | undefined;
  ...
  try {
    for (const kind of ['missing', 'retired', 'expired', 'spent', 'cancel-before', 'cancel-consumed', 'replace-before', 'aba-before', 'replace-inflight', 'aba-inflight', 'ordinary-inflight', 'ordinary-late']) {
      ...
      if (kind !== 'ordinary-inflight') jar.set('__Host-account_continuation', ref); const page = await send(kind === 'ordinary-inflight' ? '/account/signin' : '/account/signin?restart=standalone'); expect(page.status).toBe(200); const form = fields(await page.text()); if (kind !== 'ordinary-inflight') expect(form.get('restart')).toBe('standalone');
