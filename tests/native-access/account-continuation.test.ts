...
it('native first setup preserves original client state/S256 through identity, explicit repository consent and connector return', async () => {
  ...
  try {
    ...
    const identity = await send('/account/callback?code=identity&iss=https%3A%2F%2Fgithub.com%2Flogin%2Foauth&state=' + loginState); expect(identity.status).toBe(303); expect(identity.headers.get('Location')).toBe('/account/continue');
    ...
    const connected = await send('/oauth/callback?code=repository&iss=https%3A%2F%2Fgithub.com%2Flogin%2Foauth&state=' + repoState); expect(connected.status).toBe(303); expect(connected.headers.get('Location')).toBe('/account/continue');
