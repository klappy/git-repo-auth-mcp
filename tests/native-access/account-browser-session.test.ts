...
it('actual native browser sign-in/callback consumes state and emits only opaque Secure cookies', async () => {
  ...
  const calls: string[] = [];
  const mf = new Miniflare({ modules: true, script: output.outputFiles[0].text, compatibilityDate: '2026-06-16', host: '127.0.0.1', port: 0, cf: false, bindings: { BROWSER_SESSION_KEY_HEX: 'ab'.repeat(32), PRIVATE_ACTIVATION: 'owner-verified', ACCOUNT_ISSUER: 'https://fixture.invalid', ACCOUNT_LOGIN_CALLBACK: 'https://fixture.invalid/account/callback', GITHUB_CLIENT_ID: 'synthetic', GITHUB_CLIENT_SECRET: 'INERT_CLIENT' }, durableObjects: { ACCOUNT_BROWSER_SESSIONS: { className: 'AccountBrowserSessions', useSQLite: true } }, outboundService: (request: import('miniflare').Request) => {
    calls.push(request.url);
    if (request.url === 'https://api.github.com/user' && request.method === 'GET') return Response.json({ id: 1001 }, { headers: { 'x-oauth-scopes': 'read:user' } });
    if (request.url === 'https://github.com/login/oauth/access_token' && request.method === 'POST') return Response.json({ access_token: 'INERT_ACCESS', refresh_token: 'INERT_REFRESH', token_type: 'bearer', scope: 'read:user' });
    throw new Error('Synthetic provider denies unlisted destination/method');
  } });
  try {
    ...
    // Miniflare follows redirects by default, using its global dispatcher for other origins.
    // Inspect the browser redirect; do not navigate to real GitHub from this synthetic test.
    const start = await mf.dispatchFetch('https://fixture.invalid/account/signin', { method: 'POST', redirect: 'manual', headers: { Cookie: cookie, Origin: 'https://fixture.invalid' }, body: new URLSearchParams({ csrf }).toString() }); expect(start.status).toBe(303); expect(calls).toEqual([]);
    ...
    const completed = await mf.dispatchFetch(callback, { redirect: 'manual', headers: { Cookie: cookies } }); expect(completed.status).toBe(303); expect(completed.headers.get('Set-Cookie')).toMatch(/__Host-account_session=[a-f0-9]{64}; Secure; HttpOnly; SameSite=Lax; Path=\//); expect(completed.headers.get('Set-Cookie')).not.toMatch(/INERT|code_verifier/); expect(await completed.text()).not.toMatch(/INERT/);
    expect((await mf.dispatchFetch(callback, { redirect: 'manual', headers: { Cookie: cookies } })).status).toBe(503); expect(calls).toEqual(['https://github.com/login/oauth/access_token', 'https://api.github.com/user']);
